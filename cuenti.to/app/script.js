  ...
  const interval = setInterval(() => {
    ...
    if (i > 15 && i <= 25) {
      $(".message-container").html(
        "<p>Dependiendo la cantidad de gente conectada, esto puede demorar un poco...</p>"
      );
    } else if (i > 25 && i <= 35) {
      $(".message-container").html(
        "<p>Primero vas a ver tu Cuentito escrito y luego las imágenes...</p>"
      );
    } else if (i > 35 && i <= 65) {
      $(".message-container").html("");
      $(".message-container").html(
        "<p>Si el proceso tarda más de un minuto, no te preocupes. Te avisamos por mail cuando tu Cuentito esté terminado!</p>"
      );
    } else if (i > 65 && i <= 85) {
      $(".message-container").html(
        "<p>Podés editar tu Cuentito marcando el texto que quieras cambiar y agregando nuevo.</p>"
      );
    } else if (i > 86) {
    } else if (i > 95) {
  ...
  fetchRequest = fetch("/app/generateStory.php", {
    ...
    .catch((error) => {
      if (error.name === "AbortError") {
        ...
        const noticeBar = document.querySelector(".notice-bar");
        noticeBar.innerHTML =
          "Error " + error.message + ". Por favor intentar nuevamente.";
        noticeBar.style.display = "block";
        throw error;
      }
    });
  ...
  fetch("/app/prompts.txt")
    ...
    .then((data) => {
      ...
      textarea.placeholder =
        "Escribí 10 palabras que describan una situación, personajes o la idea principal de tu Cuentito. Por ejemplo:\n\n" +
        prompt;
    })
    .catch((error) => {
